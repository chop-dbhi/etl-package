...
import edu.chop.cbmi.etl.audgendb.extraction.sql.{EncounterDiagnosisExtraction, MedicalHistoryExtraction, ProblemListExtraction}
    ...
    elements.foreach {
      element =>
        ...
        while (sourceResult.next()) {
          ...
          visitDate = if (element == "medical_history") {
            //Medical History uses Varchar
          ...
          if (patientBuffer(0)._1 != patientId) {
            patientBuffer = MutableList.empty
          }
    ...
    elements.foreach {
      element =>
        ...
        while (sourceResult.next()) {
          ...
          visitDate = if (element == "surgical_history") {
            //Medical History uses Varchar
          ...
          if (!patientBuffer.contains((patientId, visitDate, procedureId))) {
            ...
            val field_1 = if (element == "surgical_history") {
              "surgical_hx.cpt_code"
            }
            else if (element == "procedure") {
              "procedure.proc_code"
            }
            else {
            ...
            val id_1 = if (element == "surgical_history") {
              sourceResult.getObject("cpt_code")
            }
            else if (element == "procedure") {
              sourceResult.getObject("proc_code")
            }
            else {
