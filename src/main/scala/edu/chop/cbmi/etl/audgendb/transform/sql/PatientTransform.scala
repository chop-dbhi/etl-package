...
case class PatientTransform(override val sdbfp: String, override val tdbfp: String,
  ...
  override def append: Boolean = {
    ...
    while (sourceResult.next()) {
      ...
      if ((sourceResult.getString("sex").trim.toLowerCase == "m")) {
        sex = "Male"
      }
      else if ((sourceResult.getString("sex").trim.toLowerCase == "f")) {
        sex = "Female"
      }
      else {
        sex = null
      ...
      if ((middleName == null) || (middleName.toUpperCase == "NULL")) {
        middleName = null
      }
  ...
  def generatePatientAlias: String = {
    ...
    for (i <- 1 to maxAliasGenerationAttempts) {
      ...
      if (!existingPatientAliases.contains(patientAlias)) {
        return patientAlias
      }
